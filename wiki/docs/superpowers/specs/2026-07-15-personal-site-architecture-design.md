...
| Decision | Rationale |
| --- | --- |
| 사이트 코드는 이 repo `app/fe`에 둔다 (8차) | 콘텐츠와 사이트가 함께 진화하고 별도 repo sync 단계가 사라진다. 분리가 필요해지면 폴더 추출 비용이 낮다. |
| 스택: Next.js + Fumadocs + assistant-ui + Vercel AI SDK | 2026-07-04 설계 승계. |
| 핵심 사이트 배포: Vercel (Root Directory=`app/fe`) + `marinkim.xyz` | 채용 담당자가 보는 경로는 zero-ops로 항상 살아 있게 한다. |
...
| 완전한 monorepo (7·8차 개정) | 서비스 코드는 `labs/{svc}/` 자립 폴더(스택 자유·각자 Dockerfile), k8s 구성·manifest는 `infra/` 소유. Dae-Jeong repo = `wiki/`(지식) + `app/`(fe·be) + `labs/` + `infra/`. 배포 경계는 CI 경로 필터. |
...
```text
wiki/ (profile·evidence·products — private evidence 포함)
        │
        └─ export 스크립트: public: true claim·public-safe 산출물만 필터
                ↓ (수동 실행, release 시)
app/fe/content/{resume,portfolio} + app/fe/public/resume.pdf
app/fe/content/blog  ← 네이티브 작성 (public-safety 체크리스트 적용)
```

- site 코드는 `app/fe` 밖의 repo 경로를 import·read하지 않는다 (특히 `wiki/` 직접 접근 금지). 콘텐츠 유입 경로는 export 스크립트 하나다.
...
2. `app/fe/content/services/{svc}.md` 하나 등록하면 사이트 노출 끝 — `/labs` 카드와 `/labs/{svc}` 상세 페이지가 frontmatter·본문에서 생성된다.
...
```yaml
# app/fe/content/services/{svc}.md frontmatter
id: stock-radar
title: Stock Radar
url: https://stock.marinkim.xyz
source: labs/stock-radar   # monorepo 폴더 (공개 추출 시 repo URL로 교체)
status: wip
summary: 한 줄 소개
...
flowchart LR
    subgraph DJ["Dae-Jeong repo (private · source of truth)"]
        EV["wiki/evidence/<br/>claim registry"]
        REG["app/fe/content/services/*.md<br/>(labs registry)"]
        SITE["app/fe<br/>Next.js core"]
    end
...
| 2 | visitor profile chat (site serverless — 6차 확정) + jarvis (`app/be` FastAPI, Render) 착수, 데이터 필요 시 Supabase 도입 | 2026-07-02/04 chat 설계 실행 |
...
- **10·11차 개정 (2026-08-13)**: 토큰 값은 불변, **적용 위치**를 조정했다.
  ...
  ⑤ ~~미결~~ → **12차 해소 (2026-08-13)**: doc 지면 섹션 제목도 `2xl(28)`로 통일.
  ⑥ **12차 — 케이스 상세 5단**: `문제·검토·결정·시스템·결과` (PAAR). `검토`(대안 카드,
  기각 사유 병기)는 선택 섹션 — 근거 없는 케이스는 생략하고 번호·rail이 동적으로 이어진다.
  `운영 근거`→`결과` 개칭 (R은 증빙이 아니라 "그래서 어떻게 됐나"에 답한다 — id `#ops`는 유지).
