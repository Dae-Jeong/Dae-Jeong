...
description: marinkim.xyz Phase 1의 publish 기술 스택과 app/ 프로젝트 구성 — Tier 2 구현 사양.
...
| export 스크립트 | Python — public: true claim 필터, `app/fe/content/{resume,portfolio,services}` + `app/fe/public/resume.pdf` 생성 | 확정 |
| 배포 | Vercel (Root Directory=`app/fe`) + marinkim.xyz, git push 자동 | 확정 (spec) |
...
- Jarvis (6차 개정): UI는 site 내장(react-force-graph·Web Speech API/kokoro-js), **backend는 `app/be` FastAPI — Render free 배포** (RAG·wiki 인덱싱·대화 메모리). 벡터·상태는 Supabase pgvector `jarvis` schema (Render Postgres 금지). cold start 30~60초 → 페이지 진입 warm ping 설계.
- 인프라 (labs 서비스 전용): k8s (위치 미정, Oracle free 후보) + ingress·cert-manager, 구성은 `infra/` 소유 — 프로필 제품(app/)은 k8s를 쓰지 않는다
...
## Repo 구성 (2026-07-18, spec 8차 개정 기준)

```text
Dae-Jeong/  ← knowledge harness + 완전한 프로젝트 monorepo
├── wiki/    # 지식 층 — profile·evidence·products·backlog·rules·docs·context·archive
├── app/     # 프로필 제품 (marinkim.xyz)
│   ├── fe/  # Next 앱 (visitor chat 프록시 포함) — Phase 1 생성, Vercel 배포
