---
type: platform-copy
title: Remember 프로필 문안
description: 리멤버 프로필의 canonical 문안 (Product Engineer v3). 커넥트용·채용 서비스용 소개 두 탭.
platform: remember
...
이 파일이 **Remember 프로필 문안의 canonical 문안**이다. 사실·강도는 `evidence/claims/`, 표현 규칙은 [copy-standard §1-6](../../rules/application-copy-standard.md), 적용 절차와 자동화 제약은 [README](README.md)와 skill `sync-platform-profile`이 소유한다. live 적용 상태는 [platform-registry.yaml](platform-registry.yaml)이 기록한다.
...
현재 MediSolve AI에서 Tech Lead · Backend Engineer로 제품 운영과 백엔드·AI 시스템을 함께 맡고 있습니다.
```
...
- 텍스트 자동 입력이 정상 동작한다(한글은 `insertText`). 두 소개 탭은 각각 저장 → reload → 재확인.
- 공개 범위와 기존 외부 활동은 바꾸지 않는다.
