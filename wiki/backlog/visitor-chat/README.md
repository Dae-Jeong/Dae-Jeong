...
- 비용 구조 확인 (2026-07-17, `wiki:pricing/vercel`): LLM 추론은 제공사 인프라 부담, Vercel 함수는 얇은 스트리밍 릴레이(Fluid Active CPU 과금 — 요청당 ~$0.0001 수준). Hobby 무료 한도로 월 수천 요청 여유, 초과 시 과금이 아니라 차단이라 요금 폭탄 없음. **실질 비용 = LLM 토큰** → 착수 시 남용 방지 설계 필수: per-IP rate limit·세션당 메시지 한도·curated fallback 우선 응답.
