...
description: 2026-08-22~24 로그인 편집 화면에서 채용 플랫폼 5곳에 확정 문안을 반영하고 저장 후 reload로 검증한 결과.
...
- **Maker v2 재동기화**: 5개 플랫폼의 소개와 MediSolve AI 경력을 `아이디어를 고객이 돈을 내는 제품으로 만드는 메이커`와 실제 고객이 결제하는 유료 제품 운영 경험으로 다시 맞췄다. 정확한 매출 band와 광고 실험은 넣지 않았다.
...
> 2026-08-24 사이트 SoT와 5개 플랫폼을 `아이디어를 고객이 돈을 내는 제품으로 만드는 메이커` 중심의 서사로 동기화했다. 정확한 매출 band와 광고 실험은 public copy에서 제외했다.

## 2. 플랫폼 요약

| 플랫폼 | 편집 | reload | 반영 범위 | 보존한 값·설정 | 잔여·결과 |
| --- | --- | --- | --- | --- | --- |
| Wanted | 완료 | 통과 | Maker v2 소개, 직무·직책, MediSolve AI 포함 기존 4개 경력, AX, skill, 학교명, 링크 | 기존 인증 경력 구조 | 더데이랩스 별도 row와 credentials는 안전한 추가 UI가 없어 미적용 |
| Remember | 완료 | 통과 | Maker v2 소개 2개 탭, MediSolve AI 포함 경력 5건, backend 직무, skill 15개 | 공개 설정과 기존 활동 | 적용 완료 |
| Groupby | 완료 | 통과 | Maker v2 소개, MediSolve AI 포함 경력 5건, skill 15개, TellingMe | 프로필 공개, 적극 구직, 희망 조건 | 적용 완료 |
| RocketPunch | 완료 | 통과 | 300자 Maker v2 소개, MediSolve AI 포함 경력 5건, STUDIO LAB 종료월, 사실 오류·금지 문구 정리 | 구직 의향과 portfolio 링크 | 저장 후 자동 AI 커리어 요약도 새 유료 제품 프레임으로 재생성됨 |
| LinkedIn | 완료 | 통과 | Maker v2 headline·About, 대표 보유기술, 웹사이트, MediSolve AI 포함 경력 6개 entry | 공개 구직 설정 | Featured는 `marinkim.xyz` 링크 검증 실패로 미등록. 상단 웹사이트 버튼은 정상 노출 |
...
| --- | --- | --- | --- | --- |
| 간단 소개 | §1.3 | `아이디어를 고객이 돈을 내는 제품으로 만드는 메이커`와 유료 제품·직접 구현 범위 반영 | 통과 | 프로필 150자 제한에는 110자 축약본 사용 |
| 현재 직무·직책 | §0, §3.1 | 검색 직무 `백엔드 개발자`·직책 `Tech Lead` 반영 | 통과 | MediSolve 경력 본문에서 Backend Engineer 수행 범위를 함께 설명 |
| MediSolve AI | §2.1 | 고객 문제→유료 제품 운영→backend·AI·Next.js 직접 구현을 시작점으로 둔 최신 경력 반영 | 통과 | 정확한 매출 band 없음 |
...
| Field | Target | Live 결과 | Reload | Note |
| --- | --- | --- | --- | --- |
| 커넥트용 소개 | §1.3.1 | Maker v2와 고객 문제·유료 제품·직접 구현을 중심으로 한 소개 반영 | 통과 | 두 탭을 별도 저장·확인 |
| 채용 서비스용 소개 | §1.3.2 | Maker v2와 현재 역할·기술 범위를 중심으로 한 소개 반영 | 통과 | 두 탭을 별도 저장·확인 |
...
| Field | Target | Live 결과 | Reload | Note |
| --- | --- | --- | --- | --- |
| 자기소개 | §1.3.3, §3.4 | Maker v2·유료 제품 운영·직접 구현과 검증 자산 묶음 반영 | 통과 | 기존 credentials 보존 |
...
| Field | Target | Live 결과 | Reload | Note |
| --- | --- | --- | --- | --- |
| 소개 | §1.2 | 300자 제한용 Maker v2 소개 반영 | 통과 | 고객 문제→유료 제품→FastAPI·AI·Next.js 직접 구현 포함 |
| 경력 5건 | §2.1–§2.6 | MediSolve AI를 Maker v2 서사로 갱신하고 전 경력 최신화 | 통과 | 정확한 매출 band 없음 |
| STUDIO LAB 종료 | §0.1 | `2024.01`로 정정 | 통과 |  |
| 금지·사실 오류 | §7 | 36시간·5xx·4,039건·Hongkong·3회 재시도·C# 제거 | 통과 |  |
| 구직 의향·portfolio | — | 기존 값 보존 | 확인 | 설정 변경 없음 |
| AI 커리어 요약·HTML title | 플랫폼 자동 생성 | 저장 후 새 경력 기반으로 재생성 | 통과 | exact revenue band 제거와 유료 제품·직접 구현 프레임 확인 |

### 3.5 LinkedIn

| Field | Target | Live 결과 | Reload | Note |
| --- | --- | --- | --- | --- |
| headline | §1.1, §3.2 | `Tech Lead · Backend Engineer` 역할과 Maker v2 문장을 함께 반영 | 통과 | 공식 경력 title은 `Backend Engineer`로 분리 |
| About | §1.4 | Maker v2·고객 문제·유료 제품·직접 구현을 중심으로 한 KO 소개 반영 | 통과 | 정확한 매출 band·`기업부설연구소장`·36시간·5xx 구문 없음 |
...
| MediSolve AI | §2.1 | title `Backend Engineer`, Tech Lead 역할·유료 제품 운영·직접 구현 성과 반영 | 통과 | 정확한 매출 band를 제거하고 Azure는 `배포 환경 구성·기본 운영`으로 낮춰 저장 후 reload 검증 |
...
4. LinkedIn Featured는 도메인 링크 검증이 정상화되기 전까지 상단 웹사이트 버튼으로 대체한다.

## 6. 결론

Wanted·LinkedIn·Remember·Groupby·RocketPunch는 2026-08-24 Maker v2 문안의 저장·reload 검증을 마쳤다. 정확한 매출 band와 광고 실험은 넣지 않았고, RocketPunch 자동 AI 커리어 요약도 새 경력 기반으로 다시 생성된 것을 확인했다.
Saramin은 대상에서 제외했다. Oopy 정리와 Wanted·LinkedIn의 안전한 UI 잔여값이 남아 있어 `wip` 상태를 유지한다.
