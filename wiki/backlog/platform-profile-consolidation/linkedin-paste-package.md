...
## 2. 경력 설명 6건 — 개조식 v3 (2026-08-12, 명사형 종결)
...
- 월 수만 건 규모에서 HTTP 5xx 0.3% 수준 운영 (30일 기준)
- 도메인 간 의존성으로 회원 로직 수정이 AI 생성 중단으로 전파, QA 티켓 종료 후에도 동일 영역 재발 반복
- 부분 수정으로는 의존성 구조 잔존으로 판단해 전면 재구축 결정 및 사내 반대 설득 (근거: 문제 누적·AI 모듈 확장 계획·하네스 선행 시 이관 속도)
- 디자인 패턴·인프라 하네스 선행 구축 후 AI 코딩 에이전트와 협업, 파악부터 재구축까지 36시간 (작업 시간 기준)

[LLM Evaluation — 생성 품질 판정 체계 구축]
- "품질이 나쁘다"로는 재생성 기준 확정 불가 — 코드가 거를 수 있는 층과 사람만 판단 가능한 층을 분리
- 자동 게이트 12종 구축, 틀린 출력을 사람 검토 이전 단계에서 차단
...
- 유형 분기 판정을 writer 배치 시 18건 전부 미발동 — writer가 원본 유형을 알 수 없는 자리로 판단
- 판정 위치를 planner로 재배치해 유형별 분기 정상화, 판단을 어느 역할에 두는가가 agent 설계의 핵심임을 실측 확인

[Centurion — 피부과 운영 CRM backend]
- order·product·inventory API와 비동기 worker 흐름 구축
- 실패 가능 작업을 제품 시작 시점부터 API 경계 밖 worker로 분리, 재고 차감에 retry 적용 (직전 회사 결제 실패 수습 경험 기반의 예방 설계)
- 메시지 계층 Celery → TaskIQ 전환 및 알림 도메인 분리
- 실시간 상담 AI 에이전트(WebSocket 기반 STT·LLM 파이프라인) 공동 주 기여로 구축 중
...
- Stripe 선결제 도입 및 Hongkong API 기반 다국가 결제·구독·환불 트랜잭션 처리
