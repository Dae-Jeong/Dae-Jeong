...
PM으로 옮겨 1년간 제품 전반을 맡았으며(2022.10~2023.09),
마지막 구간은 Backend Engineer였습니다(2023.10~2023.12).

PM 시기에는 프로토타입의 사용 데이터를 분석해 무엇을 남기고 무엇을 바꿀지 정하는 것에서
시작해, SellerCanvas v1.0 전반의 서비스 기획을 맡았습니다.
개발이 외주로 진행되던 시기라 외주 개발사의 일정과 산출물을 관리했고,
상세페이지가 만들어지는 제작 Flow 자체를 다시 설계해 개선했습니다 —
이 제작 방식은 「페이지 출력 방법」 특허 출원으로 이어져 등록까지 마쳤습니다(10-2898273).
패션 대기업 브랜드 POC에서는 비즈니스 요구사항을 기술 스펙으로 변환하며
기획·일정·기술 검증을 담당했습니다.

PM으로 있으면서도 필요하면 직접 만들었습니다 —
의류 색상 분류 모델을 개발해 분석 정확도 보정에 활용했고,
의류 분석 모델은 한국건설생활환경시험연구원(KCL)의 AI 정확도 인증을 통과했습니다.
