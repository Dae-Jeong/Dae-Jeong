...
## 2. 경력 설명 6건 — bullet 하이브리드 (2026-08-12 전면 개정)
...
- 문제: 도메인 간 의존성 — 회원 로직을 수정하면 AI 생성이 중단되는 식으로
  변경이 전파됐고, QA 티켓을 닫아도 같은 영역에서 다른 형태로 재발했습니다
- 판단: 부분 수정으로는 의존성 구조가 남아 문제가 계속 누적된다고 봤습니다.
  "돌아가는 기능을 왜 다시 만드나"라는 반대는 문제 누적, AI 모듈 확장 계획,
  하네스를 먼저 세우면 이관이 오히려 빠르다는 점으로 설득했습니다
- 범위: FE(Next.js)는 유지하고 backend만 FastAPI로 분리 교체
- 실행: 디자인 패턴과 인프라 하네스를 먼저 세운 뒤, 그 규칙 위에서
  AI 코딩 에이전트와 협업 — 파악부터 재구축까지 작업 시간 기준 36시간
- 성과: QA 티켓 reopen 비율 37% → 11% (해결 대비 reopen, cutover 전후 Jira 집계)
- 운영: 월 수만 건 규모에서 HTTP 5xx 0.3% 수준 (30일 기준)

[LLM Evaluation — 생성 품질을 판정 가능한 대상으로]
- 문제: "품질이 나쁘다"는 말로는 무엇을 고치고 언제 재생성할지 정할 수 없었습니다
- 판단: 품질을 한 덩어리로 보지 않고, 코드가 거를 수 있는 층과
  사람만 판단할 수 있는 층을 분리해야 한다고 봤습니다
- 실행: 자동 게이트 12종 — 틀린 출력은 코드가 먼저 거릅니다
- 실행: 실측 분포 대조 — 직접 수집한 코퍼스(n=19 → 4,039)와 대조합니다
- 실행: 사람 판정 — 자동화가 닿지 않는 층만 사람이 봅니다
- 운영: 반증된 프롬프트 규칙은 기록으로 남겨 같은 시도의 반복을 차단했습니다

[Agent Workflow — planner·writer 파이프라인 설계·구현]
- 범위: 평가 하네스만이 아니라, 하네스로 검증할 agent 구조 자체를
  함께 설계·구현했습니다 (대표 영역: writer)
- 문제: 유형 분기 판정을 writer에 뒀더니 18건 전부 발동하지 않았습니다
- 판단: writer는 원본이 어떤 유형인지 알 수 없는 자리였습니다 —
  판정의 실패가 아니라 판정 위치의 문제로 봤습니다
- 성과: 판정을 planner로 옮기자 유형별로 정확히 갈렸습니다.
  판단을 어느 역할에 둘 것인가가 agent 설계의 핵심임을 실측으로 확인했습니다

[Centurion — 피부과 운영 CRM backend]
- 실행: order·product·inventory API와 비동기 worker 흐름 구축
- 판단: 장애를 겪고 대응한 것이 아니라 제품 시작 시점의 예방책이었습니다.
  직전 회사에서 결제 실패가 남기는 불일치를 직접 수습한 경험을 바탕으로,
  실패 가능한 작업은 처음부터 API 경계 밖 worker에 두고 재고 차감에 retry를 붙였습니다
- 실행: 메시지 계층을 Celery에서 TaskIQ로 전환하며 알림 도메인 분리
- 진행: 상담 현장을 실시간으로 돕는 AI 에이전트(WebSocket 기반 STT·LLM
  파이프라인)를 공동 주 기여로 구축 중

[조직 표준·AX — 사람과 에이전트가 같은 규칙 위에서]
- 문제: 백엔드 인원 대비 담당 제품이 많은 구조에서는 제품 간 이동 비용이 병목입니다
- 실행: 조직 표준 FastAPI 템플릿 설계·구축 — layered architecture,
  의존성 주입, 응답·예외 규약, ADR·runbook
- 실행: 계층형 agent context와 도메인·셋업 자동화 스킬 내장 —
  AI 코딩 에이전트 협업 워크플로우를 팀에 도입
- 성과: 신규 backend가 같은 구조에서 출발하므로,
  제품 정책만 파악하면 누구든 대응할 수 있습니다
...
- 판단: 처음 꾸려지는 개발팀이라 코드보다 기준이 먼저 필요하다고 봤습니다
- 실행: 프로젝트 기본 구조와 Config 설계, SSH 터널링 lifecycle,
  환경·실행 문서, ISSUE/PR 템플릿을 저장소 생성 시점에 함께 넣었습니다
...
- 문제: 결제는 중간 단계에서 실패할 때 상태가 어긋난 채 남는 것이 문제였습니다
- 판단: 개별 버그가 아니라 실패 지점마다 정의가 빠진 상태 전이 문제로 봤습니다
- 실행: Stripe 선결제 도입, Hongkong API로 다국가 결제와
  구독·환불의 트랜잭션 상태 처리
- 실행: 선결제 예약 실패 시 롤백, 환불 처리 순서, 환불 시 티켓 제거 시점,
  전액 마일리지 결제 건의 환불 — 실패 지점마다 남는 불일치를 하나씩 정의했습니다
- 운영: 알림톡은 발송 실패 시 3회까지 자동 재시도
...
- 실행: 기획부터 API·DB 설계, 구현까지 0에서 구축
- 실행: 회원 도메인 설계 (이메일/비밀번호 인증, JWT 세션 관리)
- 이후: 1개월 후 정규직으로 전환했습니다
```
...
AI Engineer(2021.12~2022.09) → PM(2022.10~2023.09) →
Backend Engineer(2023.10~2023.12) 순으로 역할이 바뀌었습니다.

[PM — 제품 전반 1년]
- 시작: 프로토타입의 사용 데이터를 분석해 무엇을 남기고 무엇을 바꿀지
  정하는 것에서 출발했습니다
- 실행: SellerCanvas v1.0 전반의 서비스 기획을 맡았고, 개발이 외주로
  진행되던 시기라 외주 개발사의 일정과 산출물을 관리했습니다
- 성과: 상세페이지가 만들어지는 제작 Flow 자체를 다시 설계했고,
  이 제작 방식은 「페이지 출력 방법」 특허 출원으로 이어져 등록까지 마쳤습니다
  (등록 10-2898273)
- 실행: 패션 대기업 브랜드 POC — 비즈니스 요구사항을 기술 스펙으로 변환하며
  기획·일정·기술 검증을 담당했습니다

[PM으로 있으면서도 직접 만든 것]
- 실행: 의류 색상 분류 모델을 개발해 분석 정확도 보정에 활용했습니다
- 성과: 의류 분석 모델이 한국건설생활환경시험연구원(KCL)의
  AI 정확도 인증을 통과했습니다

[Backend — 레거시 이관]
- 실행: Node.js 레거시를 NestJS로 옮기며 예외 처리 구조를 세웠습니다
- 성과: JWT 인증과 Controller→Service→DAO 흐름을 담은 팀 개발 템플릿을
  만들었습니다 — 지금 조직 표준 템플릿을 설계하는 일의 첫 형태였습니다
...
- 실행: 인식 모델 개발, 학습 데이터 수집·관리, 영상 데이터 전처리 파이프라인
- 실행: Python·FastAPI 기반 백엔드
- 역할: 인턴이었지만 기능 요구사항 정의와 일정·태스크 관리까지 맡았습니다 —
  이 시기에 이미 만드는 일과 정하는 일을 병행했습니다
```
