...
```
AI 제품을 만드는 일만 4년째 하고 있습니다.
만드는 일과, 무엇을 왜 만들지 정하는 일을 함께 맡습니다.

모델을 만들다 기획으로, 다시 백엔드로 자리를 옮겨 왔습니다.

백엔드를 택한 이유는 분명합니다.
AI가 구현을 점점 더 많이 맡게 될수록,
보안과 안정성처럼 사람이 끝까지 책임지는 층의 무게가 커진다고 봤습니다.
지금 AI와 협업해 제품을 만들면서 그 생각은 더 확고해졌습니다.

"품질이 나쁘다"로는 고칠 대상을 정할 수 없습니다.
그래서 AI 생성 품질을 판정 가능하게 만들었습니다.

▪ 자동 게이트 — 틀린 출력은 코드가 먼저 거른다
▪ 실측 분포 대조 — 플랫폼다운지를 데이터로 판정한다
▪ 사람 판정 — 자동화가 닿지 않는 층만 사람이 본다
