...
```
AI 제품 백엔드를 만들고, 뭘 왜 만들지도 같이 정하는 4년차 백엔드 엔지니어입니다.

만들다 넘어온 제품을 받아서 다시 세우는 일을 해왔습니다. 그럴 때 버그 목록부터 세지 않고 문제 정의가 맞는지부터 봅니다. "품질이 나쁘다"로는 뭘 고쳐야 할지 알 수 없으니, 먼저 판정할 수 있는 형태로 바꿔둡니다.

한 번 푼 문제는 팀이 다시 쓸 수 있게 남깁니다. 지금은 백엔드를 만들면서 제품팀 운영도 같이 리드하고 있습니다.
```

3문단 · 238자. **리멤버·그룹바이·원티드·로켓펀치에 같은 본문을 쓴다.**

#### v3 재작성 (2026-08-10 야간) — "AI가 쓴 글 같다"

user feedback: 플랫폼에 올린 글이 AI 문체로 읽힌다.
**직전안(v2)은 아래에 원문 보존한다** — 되돌릴 때 쓴다.
...
**AI 문체 진단 — 고칠 때 이 목록을 기준으로 한다**

| 증상 | v2 | v3 |
| --- | --- | --- |
| 문장 길이가 균일하고 전부 `~합니다`로 끝난다 | 6문장이 모두 비슷한 길이 | 짧은 문장을 섞는다 |
| 중간점(`·`) 밀도가 높다 | `설계·구축`, `환불·마일리지·티켓`, `기업부설연구소장·Tech Lead·PO` | 조사로 풀어쓴다 (`환불과 마일리지, 티켓 롤백`) |
| 동사가 죽어 있다 (*명사 + 했습니다*) | `분리했습니다`, `수립했습니다`, `기획·구축했습니다` | `워커로 뺐습니다`, `기준을 세웠습니다` |
| 추상 명사 조합 | `컨텍스트 전환 비용`, `실패 가능한 작업을 API 경계 밖으로 분리` | 무슨 일이 벌어지는지 풀어 쓴다 |
| 대구가 지나치게 깔끔하다 | `무엇을 왜 만들지도`, `만드는 자리와 정하는 자리` | 1회로 줄이고 착지에만 남긴다 |
| **왜 그렇게 했는지가 없다** | 결과만 나열 | 판단 이유를 한 줄씩 넣는다 |

마지막 항목이 가장 크다. `실패 가능한 작업을 API 경계 밖으로 분리했습니다`는 결과만 있고,
`응답을 붙잡고 있는 것보다 나중에 다시 시도할 수 있는 게 중요하다고 봤습니다`가 붙어야 사람 글이 된다.
**영어 소문자 섞기(`backend`, `cutover`, `worker`, `template`)도 밀도가 높으면 기계적으로 읽힌다** — 한글로 쓴다.

⚠️ **v2 본문이 이미 리멤버·그룹바이·링크드인에 나가 있다.** v3 전파 대상이다.
