...
- Code-/test-backed (2026-08-28 재검증): 자동 발주 engine은 주문 생성 transaction을 먼저 끝낸 뒤 알림 policy를 호출한다. 병원에는 주문 묶음 알림을, 공급사에는 주문별 카카오 알림을 TaskIQ worker로 보내며, 공급사 알림이 모두 끝나면 성공 시 주문을 `CREATED → PENDING`, 하나라도 최종 실패하면 `CREATED → FAILED`로 바꾼다. 실패 주문은 전용 API에서 조건을 검증한 뒤 수동 재발송할 수 있다.
