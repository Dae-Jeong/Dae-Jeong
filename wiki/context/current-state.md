...
- **회사 AX 전환 설계로 확장** (2026-08-21): 제품 기획·개발·QA·릴리스뿐 아니라 의사결정·회의·업무 배정·승인·후속 작업을 agent-readable context·tool·human gate로 연결하는 회사 업무 AX 구조 설계 참여를 별도 claim으로 적재했다. 공통 이력서에서는 MEDINESS의 `설계 참여·제품 적용 운영 리드`를 Career와 Skills에 유지하고, 직군별 AX/FDE·Tech Lead 지원본에서 비중을 높인다. 공통 Selected Impact는 반복된 backend 전문성을 더 선명하게 보여주기 위해 03을 Centurion 비동기 복구 사례로 교체했다. 포트폴리오는 Company AX를 독립 primary case로 유지하되 current product operation과 확장 설계 중인 company-work lane을 구분한다.
...
- **공통 대표 성과 1차 확정** (2026-08-22): general resume의 `대표 성과`를 `Thready 유료 제품 운영·직접 구현`, `인계받은 backend의 FastAPI 병렬 재구축·validation harness·QA reopen 26%p 관측`, `Centurion 주문·재고 비동기 복구·test/CI 기반`, `Thready 제품 원장/AI 실행 분리·STG migration·Outbox`, `조직 표준 Backend Template·agent context`, `Azure 6-state change gate` 순으로 고정했다. 회사 AX는 Career·Skills·role variant와 portfolio primary case에 유지한다. HTTP 5xx·36시간·정확한 Threads corpus 수치는 common headline에서 제외한다.
...
- **플랫폼 프로필 1차 적용** (2026-08-23): 홈페이지·공통 resume에서 파생한 [붙여넣기 패키지](../backlog/platform-profile-consolidation/2026-08-22-platform-paste-package.md)를 Wanted·LinkedIn·Remember·Groupby·RocketPunch에 반영하고 저장 후 reload 검증을 마쳤다. LinkedIn은 headline·About·대표 기술·웹사이트·경력 6개 entry를 최신화했고, Featured는 도메인 링크 검증 실패로 상단 웹사이트 버튼이 역할을 대신한다. Saramin은 사용하지 않기로 확정해 대상에서 제외했다. Oopy 정리와 Wanted·RocketPunch의 UI 잔여값은 남아 있다. 현재 플랫폼별 상태는 [Live 적용 검증](../backlog/platform-profile-consolidation/2026-08-22-live-verification.md)이 소유한다.
...
- **포트폴리오 V3 단일 문서 반영** (2026-08-21): `/portfolio`는 네 primary case를 `Thready 제품 0→1·매출` → `Company AX 전환 설계` → `Centurion service별 failure boundary` → `회사 Infrastructure runtime·change gate` 순으로 보여준다. Company AX 사례는 MEDINESS의 제품·회사 업무 control plane, 직접 구축한 Backend Template·agent context, Azure/Vercel production 실행 근거를 contribution strength별로 분리한다. Memento payment는 supporting case로 유지하고 외부 UX 활동은 common resume에만 둔다. claim·limits·archive 같은 내부 검증 표현은 evidence/case library에만 보존한다.
...
- **Thready·SAY reference architecture 시각화** (2026-08-22): Thready는 Browser·Vercel·FastAPI product API·제품 원장/파일 저장·외부 콘텐츠 API와 GitHub Actions·Azure 배포·관측을 하나의 구조도로 연결하고, 독립 AI application·DB는 `구현·STG 검증` 상태로 분리했다. SAY는 Express Gateway·NestJS SSO·WebSocket session orchestrator·STT/event/판단 pipeline과 provider benchmark·E2E replay·regression test를 한 장에 배치하고, DELTA·COMPLETE·optional CORRECTED와 stop guard는 같은 이미지의 확대 영역으로 표현했다. 현재 운영과 검증 상태, 공동 기여 경계를 시각적으로 구분하며 자동 학습·Prod AI cutover·전체 system 단독 구축은 주장하지 않는다.
