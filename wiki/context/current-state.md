...
- **병역 이력 추가** (2026-09-08): 사용자 확인 병역을 [경력 원장](../profile/career.md#military-service)과 `career.military-service` claim에 적재하고 공용 영문 CV의 별도 `Military Service`에 반영했다. 직장 경력·실무 연차와 분리하며 기존 제출본은 유지한다. 선별 규칙·검증은 [Application Copy Standard](../rules/application-copy-standard.md) 게이트 35가 소유한다.
