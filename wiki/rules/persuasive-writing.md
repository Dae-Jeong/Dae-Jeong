...
- **측정 기준 (2026-08-13)**: 한글 기준 **110자**가 대략 2행이다. 넘으면 쪼갠다.
  쪼갤 수 없으면 사실 하나를 빼서 다른 지면(경력·포트폴리오)에 맡긴다 —
  한 줄에 사실 세 개를 밀어 넣지 않는다.
- **사례가 3건 이상 나열되면 문단이 아니라 라벨-값 목록으로 만든다.**
  각 줄이 `짧은 라벨 — 한 줄 설명`이면 스캔이 된다.
- **성과는 핵심 결과만 1줄.** 근거·측정 정의는 괄호로 붙인다.
- **상단 우선** — 최신 경력 첫 블록의 첫 줄에 최강 수치를 놓는다.
...
- 전체 분량: **이력서 2장** (경력기술서를 합쳐도 4장 이내). 인사담당자 증언상
  3장을 넘기면 읽기를 포기하는 경우가 있으므로 이력서 단독은 2장을 기준으로 한다.
...
- 개별 프로젝트의 깊이는 **포트폴리오**가 담당한다. 이력서는 그리로 넘기는 관문이다.
