...
## 15. 성향은 형용사가 아니라 사례 3건으로 쓴다 (2026-08-13 user-confirmed)

> "회사가 이런 문제가 발생했을 때, 주도적으로 문제를 해결하는 그 성향을 좀 봐줬으면 좋겠어"

정당한 요구지만 **"주도적입니다"라고 쓰면 그 순간 무효가 된다** — §2가 금지한 클리셰다.
성향은 선언으로 전달되지 않는다. **여러 사례가 같은 모양일 때** 읽는 사람이 스스로 결론을 낸다.

### 쓰는 법

1. **형용사를 지운다.** 주도적·적극적·열정적·책임감 — 전부 §7 정성 표현 금지 대상이다.
2. **같은 모양의 사례를 3건 이상 모은다.** 2건은 우연으로 읽히고, 3건부터 패턴이 된다.
3. **각 사례에 "누가 시켰나"를 드러낸다.** 성향이 보이는 지점은 결과가 아니라 **착수 계기**다.
   - 요청받았나 / 스스로 발견했나
...
- 사례가 2건 이하면 쓰지 않는다. 억지로 채우면 각 사례의 밀도가 떨어진다.
- 사례는 **서로 다른 회사·시기**일수록 강하다. 한 프로젝트에서만 나오면 상황이 특수했던 것으로 읽힌다.
- 성향 항목은 **사실 섹션 뒤**에 둔다. 근거를 본 뒤라야 패턴으로 읽힌다.
