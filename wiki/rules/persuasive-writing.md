...
## 10. 경력 설명은 bullet 하이브리드 v2로 쓴다 (2026-08-12 user-confirmed)

> "bullet 이런걸 좀 적극 활용해야 보이는 데이터가 확실하게 어떤 성과가 있고 한지
> 인사쪽 사람들이 보고 이해하기 좋을것 같아"

읽는 사람은 채용 담당자다. 1차 스캔은 **6~7.4초**(아이트래킹), 국내 경력기술서도 30~60초다.
**밀집 문단은 읽히기 전에 무시된다** — 아이트래킹 히트맵에서 고정(fixation)이 잡히지 않는다.
...
**첫 2~3어절은 스캔용, em dash 뒤는 정독용.** 시선은 F자로 좌측 여백을 수직 하강하고,
그때 **읽힘이 보장되는 것은 각 줄의 첫 2~3단어뿐이다.** 그 자리에 무엇을 놓느냐가 전부다.

| bullet 성격 | 앞에 놓을 것 |
| --- | --- |
| 성과 | 숫자 (`QA 티켓 reopen 37% → 11% — …`) |
| 판단 | 결정 동사 (`전면 재구축을 결정하고 반대를 설득 — …`) |
| 실행 | 산출물 명사 (`자동 게이트 12종 — …`) |
| 문제 | 증상 (`회원 로직 수정이 AI 생성 중단으로 전파 — …`) |

### 금지

- **라벨 접두 금지** — `문제:` `판단:` `실행:` `성과:`를 문장 앞에 노출하지 않는다.
  PAR/STAR/CAR는 *무엇을 쓸지 정하는 사고 도구*이고, 채용 가이드에서도 라벨 노출은 비권장이다.
  경력 5건이면 같은 라벨 20~30개가 가장 비싼 좌측 열을 점거해 **6초 안에 얻는 정보가 0이 된다.**
- **판단 삭제 금지.** 라벨만 지우고 판단 문장은 em dash 뒤에 그대로 둔다.
  해외 표준(XYZ)은 판단을 암시로 남기라고 하지만, 국내 시장은 과정과 근거를 보고(§6),
  최상위 프레임상 이력서가 파는 것은 성과가 아니라 **재현 가능성**이다.
- **없는 수치 창작 금지.** 수치가 없는 구간은 업무 범위·처리량·협업 규모로 대체한다.

### 분량

- 블록당 bullet **3~5개** (최신 경력은 블록을 늘리되 블록 내 개수는 유지).
- bullet **1~2줄**. 2줄을 넘으면 그건 문단이다 — 소제목을 하나 더 만든다.
- **hot zone** — 상단 1/3이 1차 스캔 주목의 80%다. 최신 경력 첫 블록의 첫 bullet이
  문서 전체에서 가장 비싼 한 줄이다. 가장 강한 수치를 놓는다.
- `[소제목]`은 국내 가이드의 **프로젝트 단위 작성** 권고와 일치하며, 수직 스캔의 앵커다.
- **소개(L1)는 문단형 유지.** 관점 선언은 서사가 필요하고, bullet로 쪼개면 논지가 사라진다.

근거: [bullet impact 분석](../products/resume/research/2026-08-12-bullet-impact.md).
canonical: [dual-track-copy-draft](../backlog/platform-profile-consolidation/dual-track-copy-draft.md)
「경력 설명 — bullet 하이브리드 canonical v2」.
