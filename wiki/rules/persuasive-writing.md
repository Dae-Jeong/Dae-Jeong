...
## 6-b. 포트폴리오에서는 국내·해외가 갈리지 않는다 (2026-08-13)

§6은 "영어권은 impact, 국내는 대안 검토"로 갈린다고 판정했다. **경력기술서 기준에서는 맞다.**
다만 **포트폴리오 케이스 지면에서는 양쪽이 같은 것을 요구한다** —
해외 자료도 "the messy middle"(검토했다가 기각한 대안과 그 뒤의 사고 과정)을 핵심으로 꼽는다.

그리고 국내 자료가 Analyze를 최상위로 두는 **이유**가 우리 정체성과 같은 논리다:

> "면접관은 Action(행동)보다 Analyze(대안 검토)를 중요하게 봅니다.
> AI가 코드를 만들어도 어떤 방식을 선택하고 적용할지 판단하는 건 결국 개발자의 몫이기 때문"
> — [서치라이트](https://blog.searchright.net/developer-career-description-examples/)

→ **대안 검토를 드러내는 것은 포맷 개선이 아니라 포지셔닝의 실증이다.**

Analyze 한 줄 패턴: `분석 방법 + 기각한 대안 + 선택 이유`.
예 — "DB 쿼리 병목·캐시 미적용 구간 분석, Memcached 대비 데이터 구조 유연성 고려해 Redis 선택".
길게 쓰지 않는다. 근거 없으면 **비운다** (§7).

근거: [케이스 서술 구조 검토](../products/site/2026-08-13-case-narrative-research.md)
...
**소제목은 프로젝트명이 아니라 역량이다** (2026-08-12 user-confirmed).
`[Thready]` `[Centurion]`처럼 프로젝트를 목차로 삼으면 회사 안에서도 주객이 전도된다 —
이력서는 프로젝트 목록이 아니라 **내가 무엇을 해줄 수 있는가**의 문서다.
관련 프로젝트는 소제목 뒤에 병기하고, 한 축이 여러 프로젝트를 걸칠 때만
필요한 줄 끝에 `· 프로젝트명`을 단다 (모든 줄에 달면 라벨 반복 문제가 재현된다).
축은 **회사마다 다르다** — 그 회사에서 실제로 한 일이 축이 된다.
...
- block당 줄 수와 bullet당 행 수를 고정하지 않는다. 한 bullet은 하나의 판정 단위를 갖고, 내용이 길면 사실을 삭제하지 말고 판단 단위로 나눈다.
- **측정 참고값**: 한글 기준 약 110자는 대략 2행이다. acceptance cap이 아니라 가독성 경고로만 쓴다.
  넘으면 문제·선택·구현·검증 단위로 bullet을 나누며, 기술 판정에 필요한 사실을 portfolio로 밀어내지 않는다.
- **사례가 3건 이상 나열되면 문단이 아니라 라벨-값 목록으로 만든다.**
  각 줄이 `짧은 라벨 — 한 줄 설명`이면 스캔이 된다.
- **case opener는 핵심 결과를 먼저 쓴다.** 이어지는 bullet에서 제약·선택·구현·검증·측정 정의를 완결한다.
- **상단 우선** — 최신 경력 첫 block에 가장 강한 verified technical proof를 놓는다. 수치가 필수는 아니다.
- **직함·회사·기간이 bullet보다 먼저다.** TheLadders 원문상 6초 리뷰 시간의 80%는
  이름·현 직함/회사·전 직함/회사·기간·학력 6개에 쓰인다. 나머지 본문은 원문 표현으로 "filler"다.
  bullet 규칙은 6초를 통과한 **2차 정독**을 위한 것이지, 6초를 이기기 위한 것이 아니다.
- 전체 분량에 고정 page cap을 두지 않는다. **첫 장**은 직함·회사·기간·최강 기술 근거가 빠르게 스캔돼야 한다.
- 이후 페이지는 완결된 기술 사례나 검증 근거로만 확장한다. 같은 주장 반복, 제품 소개, 형용사, stack 나열로 늘어난 페이지는 제거한다.
- 추가 페이지의 acceptance 기준은 길이가 아니라 `문제·제약·선택·구현·failure mode·검증·운영·결과` 중 새로운 판정 근거를 제공하는가다.
...
> "백엔드 입장에서 시스템을 전면 갈아엎은게 과연 얼마나 메리트가 있을까?
> 오히려 사이드 이팩트나 이런걸 더 걱정할것 같은데"

정확한 우려다. 경력 있는 백엔드 리더에게 rewrite는 **가장 유명한 실패 패턴**이고,
방어 없이 쓰면 "돌아가는 걸 못 참는 사람"으로 읽힌다. 기본값이 마이너스다.

**성과로 쓰지 말고 리스크 통제 증거로 써라.** 네 가지를 같은 자리에 붙인다:

1. **범위 한정** — "전면"을 단독으로 쓰지 않는다. 무엇을 유지했는지 함께 쓴다
   (예: backend만 교체, FE는 유지). blast radius를 그었다는 증거다.
2. **시점 판단** — 왜 지금인가. "서비스가 작을 때가 가장 싸다"는 조건부 판단은
   아무 때나 갈아엎는 사람이 아니라는 뜻이다.
3. **검증 선행** — 빠른 소요 시간 옆에는 반드시 하네스·테스트를 붙인다.
   숫자만 두면 "졸속"으로 읽힌다.
4. **위험 미현실화 지표** — 재작성의 전형적 실패 모드는 새 결함 폭증이다.
   결함 지표가 오히려 좋아졌다면 그것은 **개선 지표가 아니라 리스크가 현실화되지
   않았다는 증거**다. 재구축 문장과 **같은 줄 또는 바로 옆**에 둔다. 떼어놓으면 방어가 안 된다.

**점진 개선 이력을 같은 축에 병기한다.** 마이그레이션·이관·표준화 이력이 함께 보이면
재작성이 선택지 중 하나였다는 뜻이 되고, 인상이 "rewrite 애호가"에서
**"부채를 언제 갚을지 계산하는 사람"**으로 바뀐다.

## 12. 첫 화면은 답이 아니라 질문을 남긴다 (2026-08-13 user-confirmed)

> "어떤걸 중점적으로 어필해야 딱 이목을 끌고 그 내용이 궁금해서 들어가게 되는지"

스크롤을 멈추게 하는 것은 성과의 **크기**가 아니라 **결손감**이다 — "어? 그건 어떻게 했지?".
성과 나열은 정확해도 클릭 이유를 만들지 않는다.

- **실패·발견을 앞에 둔다.** 성공 나열은 누구나 쓰지만 "내가 틀렸던 지점"은 겪은 사람만 쓴다.
  가장 강한 후킹은 반직관적 사실이다 (측정을 믿었는데 측정이 틀렸다 / 재구축했는데 결함이 줄었다).
- **각 줄은 답을 다 주지 않는다.** 답은 상세 지면에 두고, 첫 화면은 질문만 남긴다.
- **신뢰 신호(직함·병행·수상)는 뒤로.** 후킹이 먼저 걸리고 나서 검증받는 순서다.
- 단 **직함·회사·기간은 이 규칙과 다른 층**이다 — §10대로 항상 최상단에 별도 노출한다.

## 13. 스택은 주력·병행·개인 프로젝트를 나눠 표기한다 (2026-08-13 user-confirmed)

나열만 하면 읽는 사람이 비중을 알 수 없고, **AI 스크리너는 아무 스킬이나 잡아 포지션을 매칭한다.**
실제로 원티드 AI 리뷰가 개인 프로젝트 스택(Java·Spring Boot) 때문에 자바 포지션 기준으로 실행됐다.
...
- **개인 프로젝트 스택을 실무와 같은 층에 두지 않는다.** 면접에서 바로 무너진다.
- **지우지 말고 범위를 밝힌다.** 숨기면 스킬란과 경력 서술이 어긋나 보이고,
  "개인 프로젝트로 만들어 출시했다"는 사실 자체는 플러스다.
- **반증된 스킬은 즉시 제거한다.** 문안의 사실 오류와 같은 급이다.
...
읽는 사람은 **회사와 역할**로 판단한다. 내부 프로젝트 코드명은 그들에게 **정보량이 0**이다.

- **사내 코드명을 그대로 노출하지 않는다.** `Thready` → `AI 콘텐츠 생성 제품`,
  `Centurion` → `AI 메디컬 플랫폼`, `SellerCanvas` → `커머스 AI 제품`.
  코드명이 필요하면 회사 설명 맨 위 「담당 제품」 한 줄에서 한 번만 매핑한다(§10).
- **경력 섹션이 가장 앞에 온다.** TheLadders 원문상 리뷰 시간의 80%가
  이름·현/전 직함·회사·기간·학력에 쓰인다. 역량·방법론 섹션보다 앞이다.
- **회사마다 "무슨 역할로 무엇을 책임졌나"를 먼저 쓴다.** 프로젝트 나열이 아니라
  **담당 범위**다. 예: "제품 backend 전반을 담당 — 재구축·평가 체계·에이전트 파이프라인·
  어드민·인프라까지 제품이 도는 데 필요한 층을 맡는다".
- 대표 기술 사례는 **이력서 안에서 자립적으로 완결**한다. 포트폴리오는 diagram·상세 evidence·긴 failure analysis로 검증을 확장한다.

역량 축(§10)은 여러 회사를 가로지르는 **주장**이고, 경력은 **사실**이다.
사실이 먼저 오고 주장이 뒤따른다 — 순서가 바뀌면 검증할 근거 없이 주장부터 읽게 된다.

## 15. (권고) 성향은 형용사보다 사례로 쓰는 편이 낫다 (2026-08-13 user-confirmed)

> "회사가 이런 문제가 발생했을 때, 주도적으로 문제를 해결하는 그 성향을 좀 봐줬으면 좋겠어"

> ⚠️ **이 절은 강제 규칙이 아니라 권고다** (2026-08-13 user 결정). 상황에 따라 달리 써도 된다.
> 다만 형용사 나열(§7 정성 표현 금지)만은 다른 절의 강제 사항으로 남는다.

"주도적입니다"는 §2가 지적한 클리셰에 가깝다 — 누구나 쓰기 때문에 변별이 없다.
성향은 선언보다 **여러 사례가 같은 모양일 때** 읽는 사람이 스스로 결론 내는 쪽이 강하다.

### 권장 방식

1. **형용사에 기대지 않는다.** 주도적·적극적·열정적·책임감은 §7 대상이다.
2. **같은 모양의 사례를 3건쯤 모으면 좋다.** 2건은 우연으로 읽히기 쉽다.
3. **각 사례에 "누가 시켰나"를 드러내면 강해진다.** 성향이 보이는 지점은 결과보다 **착수 계기**다.
   - 요청받았나 / 스스로 발견했나
   - 문제가 이미 보고됐나 / 보고되기 전이었나
   - 반대가 있었나

### 실제 적용 예 — 이력서 「일하는 방식」 04

```
문제가 티켓이 되기 전에 잡는다

재구축은 요청받은 일이 아니었고 반대가 있었다.
품질 기준값이 자사 출력을 되먹이던 순환은 아무도 문제 제기하지 않은 상태에서
재측정하다 발견했다.
주문·재고 worker 분리는 장애가 나기 전의 예방책이었다.
조직 표준 template은 내 제품이 아니라 팀의 이동 비용을 보고 만들었다.
```

형용사 0개, 사례 4건. **"주도적"이라는 말 없이 주도성이 읽힌다.**

### 참고

- 사례가 적으면 억지로 채우기보다 생략하는 편이 낫다 — 밀도가 떨어진다.
- 사례가 **서로 다른 회사·시기**면 더 강하다. 한 프로젝트에서만 나오면 상황이 특수했던 것으로 읽힌다.
- 성향 항목은 **사실 섹션 뒤**가 자연스럽다. 근거를 본 뒤라야 패턴으로 읽힌다.
