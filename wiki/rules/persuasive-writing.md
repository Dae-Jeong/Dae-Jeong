...
## 10. 경력 설명은 bullet 하이브리드로 쓴다 (2026-08-12 user-confirmed)

> "bullet 이런걸 좀 적극 활용해야 보이는 데이터가 확실하게 어떤 성과가 있고 한지
> 인사쪽 사람들이 보고 이해하기 좋을것 같아"

읽는 사람은 채용 담당자다. 문단 블록은 **읽히기 전에 스킵된다.** 판단 서사를 지키면서
스캔 가능하게 만드는 유일한 방법이 bullet 하이브리드다.

```
[소제목 — 무엇을 한 구간인가]
- 문제: 무엇이 문제였나
- 판단: 어떤 현상을 예상했고 무엇을 근거로 결정했나
- 실행: 무엇을 만들었나
- 성과: 무엇이 달라졌나 (측정 정의 포함)
```

지켜야 할 것:

- **판단 줄을 비우지 않는다.** 원티드 AI 리뷰가 제안한 3줄(문제/해결/성과)로 압축하면
  스캔성은 오르지만 재현 가능성이 사라진다 — 최상위 프레임과 정면 충돌한다.
- 라벨은 고정 틀이 아니다. 구간에 없으면 비우고, 필요하면 `운영`·`범위`·`역할`·`수상`으로 바꾼다.
  4줄을 채우려고 없는 판단을 지어내지 않는다.
- **한 bullet은 한 호흡.** 두 문장을 넘기면 bullet이 아니라 문단이다 — 소제목을 하나 더 만든다.
- 소제목 `[ ]`는 10초 스캔의 목차다. 프로젝트명·구간명처럼 **눈으로 잡히는 단위**로 쓴다.
- **소개(L1)는 문단형을 유지한다.** 관점 선언은 서사가 필요하고, bullet로 쪼개면 논지가 사라진다.
  단 소개 안의 열거(3층 판정 등)는 bullet로 뺀다.

적용 범위: L2(경력 설명) 전체 — canonical은
[dual-track-copy-draft](../backlog/platform-profile-consolidation/dual-track-copy-draft.md)의
「경력 설명 — bullet 하이브리드 canonical」 블록.
