...
const stages = [
  { key: "automated", label: "자동 검사", owner: "시스템 확인", question: "틀리지 않았는가" },
  { key: "measurement", label: "실제 결과 대조", owner: "실측 확인", question: "기준에 부합하는가" },
  { key: "human", label: "최종 판단", owner: "사람이 결정", question: "실제로 통하는가" },
] as const;
...
export function QualityDecisionDiagram({
  ...
  return (
    <figure
      ...
      className="m-0 mt-5 break-inside-avoid border-y border-[#cbd5e1] bg-white px-6 py-7"
    >
        ...
        {stages.map((stage, index) => (
          <section key={stage.key} className="relative min-w-0">
            {index > 0 && (
              <span
                aria-hidden="true"
 className="absolute -left-3 top-1/2 z-10 flex h-6 w-6 -translate-y-1/2 items-center justify-center bg-bg font-mono text-[#2854d7] max-lg:-top-3 max-lg:left-1/2 max-lg:-translate-x-1/2 max-lg:translate-y-0 max-lg:rotate-90 print:-left-3 print:top-1/2 print:translate-x-0 print:-translate-y-1/2 print:rotate-0"
              >
                →
              </span>
            )}
              <div className={`h-full border-y border-r border-[#cbd5e1] px-5 py-5 first:border-l max-lg:border-x max-lg:border-b-0 max-lg:last:border-b print:border-y print:border-r print:px-3 print:py-3 print:first:border-l ${index === 1 ? "bg-[#f8fafc]" : "bg-white"}`}>
              <div className="flex items-baseline justify-between gap-3 border-b border-border-soft pb-3">
                <div>
                  <span className="font-mono text-[11px] text-muted">{stage.owner}</span>
                  <h5 className="m-0 mt-1 text-base font-semibold text-[#102044]">
                    {stage.label}
                  </h5>
                </div>
      ...
      <figcaption className="mt-4 grid grid-cols-[148px_minmax(0,1fr)] border-y border-border-soft py-4 text-sm leading-[1.65] max-sm:grid-cols-1 max-sm:gap-2">
        <strong className="font-mono text-xs text-[#102044]">최종 판단</strong>
