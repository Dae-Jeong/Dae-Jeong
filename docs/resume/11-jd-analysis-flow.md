---
type: jd-analysis-flow
title: JD Analysis Flow
description: 한국 채용 플랫폼(원티드/사람인/로켓펀치/그룹바이/링크드인)에서 대상 직군 JD의 특징을 추출해 profile 대비 gap map을 만드는 flow 설계.
timestamp: 2026-07-05
tags: [jd, recruiting, gap-analysis, flow]
---
...
채용 플랫폼의 현재 JD들에서 대상 직군의 요구 특징(스택, 자격, 우대, AI/agent 요구)을 추출하고, [../../profile/capabilities.md](../../profile/capabilities.md)의 "Gap Candidates For JD Analysis"(현재 가정 상태)를 실측으로 교체한다.
...
6. 반영     이력서 흐름(10번 문서)의 강조점/키워드 조정
```
...
| 산출물 | 위치 |
| --- | --- |
| raw JD | `docs/resume/jd-corpus/` (플랫폼별 하위 폴더, gitignore — 로컬 전용, 2026-07-05 확정) |
| JD feature map | `docs/resume/12-jd-feature-map.md` |
| gap map + 이력서 조정안 | `docs/resume/13-jd-gap-map.md` |
...
| profile 스킬 데이터 | `scripts/jd/profile_skills.json` | 완료 — capabilities.md + claim strength 기반 |
| 리포트 생성기 | `scripts/jd/build_report.py` | 완료 — 12/13번 문서 자동 생성 |

실행: `python3 scripts/jd/collect_wanted.py 20 && python3 scripts/jd/build_report.py`
...
3. **Phase 3 — gap map**: [13-jd-gap-map.md](13-jd-gap-map.md) 생성 완료 — capabilities.md 가정 4개 중 3개 실측 검증됨. 10번 문서의 bullet 강조점 조정에 반영.
...
- 기존 `~/personal-workspace/jd_crawl/jd-drafts/`의 AX JD 3건(forward-deployed AX engineer 등)은 직군 3의 시드 샘플로 재사용한다.
